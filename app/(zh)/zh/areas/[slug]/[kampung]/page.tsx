...
export default async function KampungPageZH({
  ...
  const uniquenessMatrix = buildKampungUniquenessMatrix(k, parentArea, "zh");
  ...
  return (
    <>
                ...
                {uniquenessMatrix.serviceLinks.map((service) => (
                  ...
                  >
                    ...
                    <span className="mt-3 inline-flex items-center gap-1 text-xs font-black uppercase tracking-widest text-sky-700 group-hover:gap-2 transition-all">
                      查看服务 <FiArrowRight className="h-3 w-3" />
                    </span>
                  </NextLink>
                ))}
              </div>
      ...
      {/* Content depth §71 — local signals (ZH) */}
      ...
      <JobPhotoStrip
        ...
        place={k.name}
        hints={[k.parentSlug]}
        seed={`kampung-${k.parentSlug}-${k.slug}-zh`}
      />

      <LocalPriceComparisonTable locale="zh" name={k.name} />
      <ExpertReviewBlock locale="zh" name={k.name} context="kampung" seed={`kampung-${k.slug}-zh`} />
      ...
      <section className="py-12 bg-white border-t border-slate-100">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <Reveal>
            <MoneyCrossLinks
              block={kampungChildBlock(k.slug, k.name, slug, parentArea?.name || slug, "zh")}
            />
          </Reveal>
        </div>
      </section>
