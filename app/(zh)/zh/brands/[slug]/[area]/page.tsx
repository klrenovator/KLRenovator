...
export async function generateMetadata({
  ...
  return {
    ...
    description: ensureCtaDescription(clampMetaDescription(descText)),
    openGraph: {
      title: clampMetaTitle(titleText),
  ...
  return (
    <>
      ...
      <section className="py-12 sm:py-16 bg-white">
        <div className="mx-auto max-w-4xl px-4">
          <h2 className="text-2xl font-black tracking-tight text-slate-950 mb-3">
            如何在{area.name}维修{brand.name}冷气？
          </h2>
          ...
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-6 mb-10">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {[
                ...
                { name: "化学大修（仅挂壁式）", price: `${publishedPrices.overhaul15}起` },
          ...
          <div className="mb-10">
            <h3 className="font-black text-lg text-slate-900 mb-4 uppercase flex items-center gap-2">
              <FiMapPin className="text-sky-500" /> {`${area.name}的${brand.name}机组受哪些本地条件影响？`}
            </h3>
          ...
          {/* Content depth §71 — visit plan (ZH) */}
          ...
          {/* Content depth §71 — common jobs (ZH) */}
            ...
            <div className="mt-6 overflow-hidden rounded-2xl border border-slate-200">
              <table className="w-full text-sm">
                <thead className="bg-slate-100 text-xs font-black uppercase tracking-wider text-slate-500">
                  <tr>
                    <th className="px-4 py-2.5 text-left">工作</th>
                    <th className="px-4 py-2.5 text-left">频率</th>
      ...
      <JobPhotoStrip
        ...
        seed={`brand-area-${brand.slug}-${area.slug}-zh`}
      />
