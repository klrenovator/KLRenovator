...
const FAQS = [
  ...
  { q: "为商业客户提供年度保养合约（AMC）吗？", a: "提供 — AMC计划RM 299/年起每台，含季度预防性维护。包括优先紧急响应（2小时派遣）、15–25%维修折扣、免费诊断检查和定期保养。多台商业套餐RM 1,999/年起（5–10台）。" },
    ...
    <>
      ...
      <section className="py-20 sm:py-28 bg-slate-50" id="amc">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <Reveal delay={300}><div className="max-w-2xl mx-auto bg-indigo-600 text-white rounded-2xl p-8 text-center"><h3 className="font-black text-xl mb-2">商业AMC套餐</h3><div className="grid grid-cols-2 gap-4 mt-6 text-left"><div className="bg-white/10 rounded-xl p-4"><p className="text-xs text-indigo-200 mb-1">5–10台</p><p className="font-black text-2xl">RM 1,999<span className="text-sm font-normal text-indigo-200">/年</span></p></div><div className="bg-white/10 rounded-xl p-4"><p className="text-xs text-indigo-200 mb-1">10台以上</p><p className="font-black text-2xl">RM 3,499<span className="text-sm font-normal text-indigo-200">/年</span></p></div></div><a href={waLink("📋 AMC合约咨询\n\n您好 KL Renovator，我对商业AMC计划感兴趣。\n\n🏢 企业类型：\n🔢 台数：\n📍 地点：\n\n请发送AMC详情和价格。")} target="_blank" rel="nofollow noopener noreferrer" className="inline-flex items-center gap-2 mt-6 bg-white hover:bg-indigo-50 text-indigo-700 font-black uppercase text-sm tracking-widest px-8 py-4 rounded-xl transition-all"><FaWhatsapp className="h-4 w-4" /> 咨询AMC</a></div></Reveal>
        </div>
      </section>
      ...
      <section className="py-20 sm:py-28 bg-slate-900 text-white">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center"><Reveal>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800 flex flex-wrap justify-center gap-4">
            ...
            <Link href="/zh/services/maintenance-contract" className="text-sm text-slate-400 hover:text-white transition-colors">AMC详情</Link>
