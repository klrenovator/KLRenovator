...
const VOLUME_TIERS = [
  { units: "3 Units", price: "RM 597", save: "Save RM 0 vs individual", highlights: ["3× wall-mounted (mixed HP as needed)", "7 ft copper pipe, wiring, drain ×3", "3× vacuum pump commissioning", "1-day completion"], badge: "Popular for 3-Bedroom Homes" },
  { units: "5 Units", price: "RM 945", save: "Save RM 50 vs individual", highlights: ["5× wall-mounted (mixed HP as needed)", "7 ft copper pipe, wiring, drain ×5", "5× vacuum pump commissioning", "1-day priority completion", "Free DB box capacity assessment"], badge: "Best Value", highlight: true },
  { units: "10+ Units", price: "From RM 1,790", save: "Custom volume discount", highlights: ["10+× wall-mounted (mixed HP)", "7 ft copper pipe, wiring, drain per unit", "Vacuum pump on every unit", "Dedicated project team", "Free DB box assessment + load balancing", "Priority scheduling — complete in 1–2 days", "Custom payment schedule available"], badge: "Bulk / Commercial" },
];
...
const PROJECT_TIMELINE = [
  ...
  { phase: "Day 1 — PM", title: "Final Quote & Scheduling", desc: "You receive an itemized quote: per-unit labour, total copper pipe length, bracket types, electrical work (if any), and total project cost. Once approved, we lock in your installation date — usually within 48 hours for landed homes.", icon: <FaTags className="h-6 w-6" /> },
            ...
            {VOLUME_TIERS.map((tier, i) => (
              <Reveal key={tier.units} delay={i * 120}>
                <div className={`relative bg-white border-2 rounded-2xl p-6 sm:p-8 h-full flex flex-col ${tier.highlight ? "border-amber-400 shadow-lg shadow-amber-100" : "border-slate-200 hover:border-sky-300 hover:shadow-md"} transition-all`}>
                  ...
                  <div className="mb-4"><span className="text-3xl font-black text-sky-600">{tier.price}</span><span className="text-slate-400 text-sm ml-1">total labour</span></div>
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
          <Reveal>
            ...
            <div className="mt-8 pt-6 border-t border-slate-800 flex flex-wrap justify-center gap-4">
              ...
              <Link href="/new-home-aircond-installation" className="text-sm text-slate-400 hover:text-white transition-colors">New Home Packages</Link>
