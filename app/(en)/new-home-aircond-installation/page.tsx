...
const PACKAGES = [
  { name: "1 Bedroom (Studio / 1BR)", subtitle: "Perfect for studio apartments & 1-bedroom condos", price: "RM 199", includes: ["1× wall-mounted aircond (1.0–1.5 HP)", "7 ft copper pipe, insulation, electrical wire and drain pipe", "Vacuum pump commissioning", "1-month workmanship warranty"], badge: "Most Popular for Singles & Couples" },
  { name: "2 Bedroom (2BR)", subtitle: "Ideal for 2-bedroom condos & apartments", price: "RM 398", includes: ["2× wall-mounted aircond (1.0–1.5 HP each)", "7 ft copper pipe, insulation, electrical wire and drain pipe ×2", "Vacuum pump commissioning ×2", "1-month workmanship warranty"], badge: "Best Value for Small Families", highlight: true },
  { name: "3 Bedroom (3BR)", subtitle: "Complete cooling for 3-bedroom landed homes & condos", price: "RM 597", includes: ["3× wall-mounted aircond (1.0–2.0 HP as needed)", "7 ft copper pipe, insulation, electrical wire and drain pipe ×3", "Vacuum pump commissioning ×3", "1-month workmanship warranty", "Free site survey & electrical load check"], badge: "Whole-Home Coverage" },
  { name: "4 Bedroom (4BR+)", subtitle: "Full-house cooling for larger landed homes & semi-D", price: "From RM 796", includes: ["4+× wall-mounted aircond (mixed HP sizes)", "7 ft copper pipe, insulation, electrical wire and drain pipe per unit", "Vacuum pump commissioning on every unit", "1-month workmanship warranty", "Free site survey + DB box assessment", "Priority scheduling — complete in 1 day"], badge: "Complete Home Solution" },
];
...
const JMB_STEPS = [
  { step: 1, title: "Submit Installation Plan", desc: "We prepare a simple diagram showing outdoor unit location type, and pipe route. You submit to JMB/management office (or we help coordinate directly)." },
...
const FAQS = [
  { q: "How much does whole-house aircond installation cost for a new home in KL?", a: "A 2BR condo package (2 units) costs RM 398, a 3BR package (3 units) costs RM 597, and a 4BR+ package starts from RM 796. Each unit includes 7 ft copper pipe, insulation, electrical wire and drain pipe, vacuum pump commissioning, and 1-month workmanship warranty. Extra materials beyond 7 ft are quoted and approved on-site before work starts. All prices confirmed upfront — no hidden fees." },
  { q: "How long does new home aircond installation take?", a: "1BR (1 unit): 3–5 hours. 2BR (2 units): 5–8 hours. 3BR (3 units): 1 day. 4BR+ (5+ units): 1–2 days. We can often complete same-day for 1–3 unit installations when booked before 11 AM. Whole-house installations are scheduled in advance with priority timing." },
  ...
  return (
    <>
      ...
      <section className="relative min-h-[70vh] sm:min-h-[80vh] flex items-center justify-center bg-slate-900 overflow-hidden">
        <Image src="/logo/image.png" alt="New home aircond installation package Kuala Lumpur Selangor" fill priority sizes="100vw" className="object-cover object-center opacity-40" quality={80} />
            ...
            {PACKAGES.map((pkg, i) => (
              <Reveal key={pkg.name} delay={i * 100}>
                <div className={`relative bg-white border-2 rounded-2xl p-6 sm:p-8 h-full flex flex-col ${pkg.highlight ? "border-emerald-400 shadow-lg shadow-emerald-100" : "border-slate-200 hover:border-sky-300 hover:shadow-md"} transition-all`}>
                  ...
                  <div className="mb-4"><span className="text-3xl font-black text-sky-600">{pkg.price}</span><span className="text-slate-500 text-sm ml-1">total labour</span></div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
            {[
              ...
              { icon: <FaClipboardList className="h-5 w-5" />, title: "Material Specification", desc: "Type L copper piping, insulation 9–13mm insulation, 2.5mm²/4mm² wiring per Malaysian standards. Every material confirmed with you before work." },
