...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      images: [buildOgImage(`brand-area-${brand.slug}-${area.slug}`, `Servis aircond ${brand.name} di ${area.name} — KL Renovator`, [brand.slug, area.slug])],
    },
...
export default async function BrandAreaPageMS({
  ...
  const pageUrl = `https://www.klrenovator.com/ms/brands/${slug}/${areaSlug}`;
  const webPageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    "@id": `${pageUrl}#webpage`,
    name: `Servis Aircond ${brand.name} ${area.name} — KL Renovator`,
    url: pageUrl,
    inLanguage: "ms-MY",
  ...
  // HowTo JSON-LD sepadan dengan <ol> langkah lawatan yang dipaparkan (isu #65).
