...
const FOCUS_COPY: Record<BrandAreaComboLocale, Record<BrandFocus, { label: string; summary: string; tags: string[] }>> = {
  en: {
    inverter: {
      label: "Inverter diagnostics route",
      summary: "Best for inverter PCB checks, R32/R410A pressure balancing, indoor coil cleaning and cooling-performance testing.",
      tags: ["Inverter PCB", "R32/R410A", "Cooling test"],
    },
    commercial: {
      label: "Commercial / cassette route",
      summary: "Best for shop offices, ceiling cassette units, long run-hours, drain-line clearing and scheduled maintenance planning.",
      tags: ["Ceiling cassette", "Drain check", "AMC ready"],
    },
    family: {
      label: "Family-home service route",
      summary: "Best for terrace houses, apartments and family rooms needing chemical wash, leak checks and regular preventive servicing.",
      tags: ["Chemical wash", "Water leak", "RM99 service"],
    },
    value: {
      label: "Value-brand service route",
      summary: "Best for fast RM99 basic servicing, chemical wash, gas diagnosis and practical repair-or-replace advice.",
      tags: ["RM99 service", "Gas diagnosis", "Fast booking"],
    },
    legacy: {
      label: "Older-unit repair route",
      summary: "Best for older systems needing R22/R410A checks, capacitor diagnosis, drain clearing and honest repair-or-replace advice.",
      tags: ["Older units", "R22/R410A", "Repair check"],
    },
  },
  ms: {
    inverter: {
      label: "Laluan diagnosis inverter",
      summary: "Sesuai untuk semakan PCB inverter, balancing tekanan R32/R410A, cuci coil dalaman dan ujian prestasi cooling.",
      tags: ["PCB inverter", "R32/R410A", "Ujian cooling"],
    },
    commercial: {
      label: "Laluan komersial / cassette",
      summary: "Sesuai untuk shop office, ceiling cassette, penggunaan lama, pembersihan drain dan pelan maintenance berjadual.",
      tags: ["Ceiling cassette", "Semak drain", "AMC ready"],
    },
    family: {
      label: "Laluan rumah keluarga",
      summary: "Sesuai untuk rumah teres, apartmen dan bilik keluarga yang perlukan cuci kimia, semak bocor dan servis pencegahan.",
      tags: ["Cuci kimia", "Air menitis", "Servis RM99"],
    },
    value: {
      label: "Laluan jenama value",
      summary: "Sesuai untuk servis asas RM99, cuci kimia, diagnosis gas dan nasihat praktikal sama ada baiki atau ganti.",
      tags: ["Servis RM99", "Diagnosis gas", "Tempahan cepat"],
    },
    legacy: {
      label: "Laluan baiki unit lama",
      summary: "Sesuai untuk sistem lama yang perlukan semakan R22/R410A, diagnosis kapasitor, pembersihan drain dan nasihat baiki/ganti.",
      tags: ["Unit lama", "R22/R410A", "Semak repair"],
    },
  },
  zh: {
    inverter: {
      label: "变频诊断路线",
      summary: "适合变频PCB检查、R32/R410A压力平衡、室内盘管清洗和制冷表现测试。",
      tags: ["变频PCB", "R32/R410A", "制冷测试"],
    },
    commercial: {
      label: "商业 / 卡式机路线",
      summary: "适合店屋办公室、天花板卡式机、长时间运转、排水管疏通和定期保养安排。",
      tags: ["天花板卡式机", "排水检查", "保养合约"],
    },
    family: {
      label: "家庭住宅路线",
      summary: "适合排屋、公寓和家庭房间，需要化学清洗、漏水检查和预防性保养。",
      tags: ["化学清洗", "漏水", "RM99保养"],
    },
    value: {
      label: "经济品牌服务路线",
      summary: "适合快速RM99基础保养、化学清洗、冷媒诊断，以及实用维修或更换建议。",
      tags: ["RM99保养", "冷媒诊断", "快速预约"],
    },
    legacy: {
      label: "旧机维修路线",
      summary: "适合旧系统的R22/R410A检查、电容诊断、排水疏通，以及诚实维修或更换建议。",
      tags: ["旧机", "R22/R410A", "维修检查"],
    },
  },
};
...
const TEXT = {
  ...
  zh: {
    ...
    description: (brand: string, area: AreaRecord, focus: string, summary: string) =>
      `${focus}，适用于${area.name}, ${area.state}的${brand}冷气。${summary}`,
  },
} as const;
...
export function buildBrandAreaComboModule(
  ...
  const t = TEXT[locale];
  const combos = [...selectedAreas, ...fallbackAreas].map((area) => {
    const landmarks = area.landmarks?.slice(0, 2).join(" / ");
    const landmarkTag = landmarks || area.state;

    return {
      title: t.title(brand.name, area.name),
