// ─── AEO / GRO enhancement content: Direct Answer, DIY-safe checks,
// technician-only checks, and expanded FAQs (Round 9.5/9.8) ─────────────────
export const problemAEOContent: Record<
  string,
  {
    directAnswerEN: string;
    directAnswerMS: string;
    directAnswerZH: string;
    diyChecksEN: string[];
    diyChecksMS: string[];
    diyChecksZH: string[];
    technicianChecksEN: string[];
    technicianChecksMS: string[];
    technicianChecksZH: string[];
    extraFaqsEN: { q: string; a: string }[];
    extraFaqsMS: { q: string; a: string }[];
    extraFaqsZH: { q: string; a: string }[];
  }
> = {
  "aircond-not-cold": {
  ...
  "aircond-making-noise": {
    directAnswerEN: "Rattling or vibrating noises are usually a loose panel or mounting screw and cost little to fix; grinding or screeching noises usually mean a failing fan motor or bearing and need prompt attention before it fails completely. KL Renovator diagnoses the exact noise type before quoting.",
  ...
  "aircond-freezing-up": {
    directAnswerEN: "Ice forming on the coil is caused by either low refrigerant gas or a severely blocked evaporator coil restricting airflow — never run the unit continuously once you see ice, as it can push liquid refrigerant back into the compressor and damage it. Switch to \"Fan\" mode to let it thaw, then book a diagnosis.",
  ...
  "aircond-gas-leak": {
    directAnswerEN: "A gas leak causes cooling to fail repeatedly even shortly after a top-up — that repeat pattern is the clearest sign, versus a one-off low-gas event from natural loss over years. KL Renovator locates the exact leak point with an electronic detector before repairing the pipe or valve and re-charging.",
  ...
  "aircond-compressor-problem": {
    directAnswerEN: "A compressor that won't start, hums without spinning, or trips the breaker repeatedly usually points to a failed start capacitor first (a RM 150-250 fix) rather than the compressor itself — KL Renovator always rules out the capacitor before recommending compressor replacement, which is a much larger cost.",
  ...
  "aircond-pcb-problem": {
    directAnswerEN: "A faulty PCB (control board) typically shows up as erratic blinking lights, the unit not responding correctly to the remote, or complete shutdown despite power being present. KL Renovator diagnoses whether it's genuinely the PCB or a simpler sensor/wiring fault first, since PCB replacement is one of the pricier repairs.",
  ...
  "aircond-fan-not-working": {
    directAnswerEN: "If either the indoor or outdoor fan has stopped, the most common cause is a burnt-out fan motor (about 60% of cases) or a failed capacitor (about 30%) — both are same-day fixes. A silent outdoor unit with the indoor fan still blowing is a classic sign the outdoor fan motor specifically has failed.",
    ...
    extraFaqsEN: [{ q: "Is it safe to run the aircond if the outdoor fan isn't spinning?", a: "No — without the outdoor fan, the compressor can't dissipate heat properly and will overheat quickly, potentially causing permanent damage." }, { q: "How much does a fan motor replacement cost?", a: "Typically RM 350-480 depending on indoor/outdoor unit and brand/model." }, { q: "Why is my indoor fan working but blowing warm air along with the outdoor fan being silent?", a: "This combination strongly suggests the outdoor fan/motor has failed." }, { q: "Can dust cause a fan to stop working?", a: "Yes indirectly — heavy dust buildup adds friction that stresses a fan motor over time." }],
    extraFaqsMS: [{ q: "Selamatkah menjalankan aircond jika kipas luar tidak berpusing?", a: "Tidak — pekali tidak dapat melepaskan haba dengan betul dan akan cepat terlebih panas." }, { q: "Berapa kos penggantian motor kipas?", a: "Biasanya RM 350-480 bergantung sama ada unit dalam atau luar." }],
    extraFaqsZH: [{ q: "室外风扇不转时继续运行冷气安全吗？", a: "不安全——压缩机无法正常散热，会很快过热，可能导致永久损坏。" }, { q: "更换风扇电机需要多少费用？", a: "费用通常约RM 350-480，视室内或室外机及品牌型号而定。" }],
  },
  "aircond-tripping-power": {
    directAnswerEN: "An aircond that trips the MCB (main breaker) the moment it starts usually indicates a short circuit, failing capacitor, or compressor drawing excessive current — this is treated as an urgent electrical safety issue, not a routine repair, since a persistent short can be a fire risk.",
  ...
  "aircond-remote-not-working": {
    directAnswerEN: "In most cases the remote itself has drained batteries or a worn button contact — try fresh batteries first. If the unit still doesn't respond with new batteries and the remote's transmit light works, the issue is usually the indoor unit's IR receiver, not the remote.",
  ...
  "aircond-indoor-unit-leaking": {
    directAnswerEN: "Water dripping from the indoor unit is the single most common aircond complaint KL Renovator receives in Malaysia — nearly always caused by a blocked drain pipe or dirty drain pan due to the year-round humidity. A chemical wash or overhaul clears it, and it's rarely an electrical or refrigerant issue.",
  ...
  "aircond-outdoor-unit-not-running": {
    directAnswerEN: "If the indoor unit powers on but the outdoor unit stays completely silent, the most likely causes are a tripped overload protector, a failed compressor start capacitor, or a contactor relay fault — all are diagnosable and fixable same-day in most cases, well short of a full compressor failure.",
  ...
  "aircond-weak-airflow": {
    directAnswerEN: "Weak airflow is almost always a blockage problem, not a fan speed setting issue — a dirty filter, blocked evaporator coil, or clogged blower wheel restricts the volume of air the unit can push, regardless of fan speed setting. A chemical wash (from RM 120) resolves the vast majority of cases.",
  ...
  "aircond-not-turning-on": {
    directAnswerEN: "If the aircond doesn't respond at all to the remote or power button, check the MCB/breaker first — a tripped breaker accounts for a large share of \"won't turn on\" calls and is a free, instant fix. If the breaker is fine, the cause is usually a failed capacitor, PCB fault, or wiring issue.",
  ...
  "aircond-blinking-light": {
    directAnswerEN: "Blinking lights are your aircond's built-in error code system — the number of blinks, colour, and pattern correspond to a specific fault the manufacturer has documented. WhatsApp us the exact pattern along with your brand and model, and we can often narrow down the likely cause before we even arrive.",
  ...
  "aircond-thermostat-problems": {
    directAnswerEN: "If your aircond runs non-stop without reaching the set temperature, or turns off by itself before the room is cool, the temperature sensor is the most common culprit — it's a relatively inexpensive part to replace once confirmed faulty.",
    directAnswerMS: "Jika aircond anda berjalan tanpa henti tanpa mencapai suhu tetapan, atau mati sendiri sebelum bilik sejuk, sensor suhu adalah punca paling biasa.",
    directAnswerZH: "如果您的冷气不停运转却达不到设定温度，或在房间变凉之前自动关机，温度传感器通常是最常见的原因。",
    ...
    extraFaqsEN: [{ q: "Why does my aircond turn off by itself?", a: "Most often a faulty temperature sensor reading the room as cooler than it is, so the PCB stops the unit early. Other causes: compressor thermal overload, a frozen coil from a dirty filter, or an active timer/sleep mode. KL Renovator diagnoses and fixes it across KL & Selangor." }, { q: "Can a dirty air filter cause my aircond to shut off by itself?", a: "Indirectly, yes — heavy dust restricts airflow, the evaporator coil can freeze, and the unit cycles off on low-temperature protection until it defrosts. Clean the filter first; if auto-shutdowns continue, book a diagnosis." }, { q: "How much does a temperature sensor replacement cost?", a: "One of the more affordable aircond repairs (RM 150–250), since the sensor is a small, inexpensive component." }, { q: "Can dirty filters cause thermostat-like symptoms even with a healthy sensor?", a: "Yes — restricted airflow past the sensor location can give a false, warmer reading." }, { q: "Is a thermostat problem the same as the unit not being cold?", a: "Related but different — \"not cold\" means reduced cooling capacity, while a sensor issue means the unit doesn't know when to stop running." }, { q: "Will a smart thermostat or WiFi control module fix this?", a: "Not directly — the sensor itself needs to be tested and fixed regardless of the control interface used." }],
    extraFaqsMS: [{ q: "Kenapa aircond saya mati sendiri?", a: "Paling kerap: sensor suhu rosak membaca bilik lebih sejuk daripada sebenarnya, jadi PCB menghentikan unit awal. Punca lain: pelindung terma pekali, gegelung membeku akibat penapis kotor, atau pemasa/mod tidur aktif. KL Renovator diagnos dan baiki di seluruh KL & Selangor." }, { q: "Bolehkah penapis kotor menyebabkan aircond mati sendiri?", a: "Secara tidak langsung, ya — habuk tebal menyekat aliran udara, gegelung boleh membeku, dan unit berhenti atas perlindungan suhu rendah sehingga ia cair. Bersihkan penapis dahulu; jika masih berlaku, tempah diagnosis." }, { q: "Berapa kos penggantian sensor suhu?", a: "Antara pembaikan aircond yang lebih berpatutan (RM 150–250), kerana sensor adalah komponen kecil dan murah." }, { q: "Bolehkah penapis kotor menyebabkan simptom seperti termostat walaupun sensor sihat?", a: "Ya — aliran udara terhad boleh memberikan bacaan palsu lebih panas." }],
    extraFaqsZH: [{ q: "为什么冷气会自动关机？", a: "最常见原因是温度传感器故障，误判房间已够凉而让主板提前停机。其他原因：压缩机热保护、滤网脏堵导致结冰，或定时/睡眠模式被激活。KL Renovator 覆盖吉隆坡与雪兰莪，可上门诊断维修。" }, { q: "滤网脏会导致冷气自动关机吗？", a: "间接会——灰尘过厚阻碍气流，蒸发器可能结冰，机器在低温保护下停机直至化霜。请先清洁滤网；如仍频繁自动关机，请预约上门诊断。" }, { q: "更换温度传感器需要多少费用？", a: "这是较为实惠的冷气维修项目之一（RM 150-250），因为传感器本身是体积小、价格便宜的部件。" }, { q: "即使传感器正常，脏过滤网会造成类似温控器的症状吗？", a: "会的——传感器位置处气流受阻会给出偏高于实际温度的错误读数。" }],
  },
};