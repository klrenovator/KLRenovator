...
const STAT_LABELS: Record<Locale, string[]> = {
  en: ["Happy Klang Valley Customers", "Years HVAC Experience", "5-Star Verified Reviews", "Emergency Response Time"],
  ms: ["Pelanggan Lembah Klang Gembira", "Tahun Pengalaman HVAC", "Ulasan 5-Bintang Disahkan", "Masa Tindak Balas Kecemasan"],
  zh: ["巴生谷满意客户", "年 HVAC 经验", "五星级认证评价", "紧急响应时间"],
};

const STAT_EYEBROW: Record<Locale, string> = {
  en: "Trusted Across Klang Valley",
  ms: "Dipercayai di Seluruh Lembah Klang",
  zh: "深受巴生谷信赖",
};
...
export const StatsBand = ({ locale = "en" }: { locale?: Locale }) => {
  const labels = STAT_LABELS[locale] || STAT_LABELS.en;
  const eyebrow = STAT_EYEBROW[locale] || STAT_EYEBROW.en;
  return (
    <section className="relative bg-white border-y border-slate-100 overflow-hidden">
      ...
      <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: "radial-gradient(circle at 1px 1px, #0ea5e9 1px, transparent 0)", backgroundSize: "32px 32px" }} />
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-14 sm:py-20">
        <Reveal>
          <p className="text-center text-xs font-black uppercase tracking-[0.2em] text-sky-600 mb-10">{eyebrow}</p>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 sm:gap-8">
            {siteConfig.stats.map((s, i) => (
              <div key={s.label} className="group relative flex flex-col items-center text-center p-6 sm:p-8 bg-white rounded-2xl border border-slate-100 shadow-sm hover:shadow-lg hover:border-sky-100 transition-all duration-300">
