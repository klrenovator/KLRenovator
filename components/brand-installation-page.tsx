...
import { FaWhatsapp, FaCheck, FaHouseChimney, FaBuilding, FaSquare } from "react-icons/fa6";
...
import {
  ...
  type BrandInstallationLocale,
} from "@/config/brand-installation-content";
import { brandInstallAreaLinks } from "@/config/orphan-cross-links";
...
export function BrandInstallationLandingPage({
  slug,
  locale,
}: {
  slug: string;
  ...
  const c = getBrandInstallationContent(slug, locale);
  const basePath = locale === "en" ? "" : `/${locale}`;
  const pageUrl = `https://www.klrenovator.com${basePath}/brands/${slug}/installation`;
  const brandUrl = `https://www.klrenovator.com${basePath}/brands/${slug}`;
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: c.h1.replace(/ Aircond Installation| Pemasangan Aircond|冷气安装/, ""), item: brandUrl },
      { "@type": "ListItem", position: 4, name: c.breadcrumbLabel, item: pageUrl },
    ],
  };
  ...
  const serviceSchema = {
    ...
    brand: { "@type": "Brand", name: c.h1.replace(/ Aircond Installation| Pemasangan Aircond|冷气安装/, "") },
    offers: {
      "@type": "Offer",
      price: "199",
      priceCurrency: "MYR",
  ...
  return (
    <>
      ...
      <div className="bg-slate-50 border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-1 text-xs text-slate-500" aria-label="Breadcrumb">
            ...
            <Link href="/brands" className="hover:text-sky-600 transition">Brands</Link>
            <FiChevronRight className="h-3 w-3" />
            <Link href={`${basePath}/brands/${slug}`} className="hover:text-sky-600 transition">{c.h1.replace(/ Aircond Installation| Pemasangan Aircond|冷气安装/, "")}</Link>
            <FiChevronRight className="h-3 w-3" />
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="grid gap-12 lg:grid-cols-[1fr_0.85fr] items-start">
            <Reveal>
              ...
              <h2 className="mt-3">
                <span className={title({ size: "sm" })}>Brand-Specialist </span>
                <span className={title({ size: "sm", color: "brand" })}>Installers</span>
              </h2>
            ...
            <Reveal delay={100}>
              <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 sm:p-8">
                <h3 className="font-black text-slate-900 mb-4">{c.modelsTitle}</h3>
                ...
                <div className="mt-6 pt-6 border-t border-slate-200">
                  <h3 className="font-black text-slate-900 mb-2">{c.gasTitle}</h3>
                  <p className="text-slate-600 text-sm leading-relaxed">{c.gasBody}</p>
                </div>
              </div>
            </Reveal>
          </div>
      ...
      <ExpertReviewBlock locale={locale} name={c.h1} context="installation" seed={`brand-installation-${slug}-${locale}`} />

      <section className="py-12 bg-white border-t border-slate-100">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <MoneyCrossLinks
            block={brandInstallAreaLinks(
              slug,
