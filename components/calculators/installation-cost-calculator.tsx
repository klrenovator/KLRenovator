...
const STRINGS: Record<Lang, Strings> = {
  en: {
    ...
    wallHacking: "Wall Hacking & Concealment Work",
    wallHackingHint: `RM ${WALL_HACKING_RATE}/ft — concealed piping inside the wall`,
  ...
  ms: {
    ...
    wallHacking: "Kerja Pecah Dinding & Penyembunyian Paip",
    wallHackingHint: `RM ${WALL_HACKING_RATE}/kaki — paip disembunyikan dalam dinding`,
  ...
  zh: {
    ...
    pvcHint: "按实际线槽长度收费",
    wallHacking: "打墙与隐藏式走管工程",
    wallHackingHint: `每英尺 RM ${WALL_HACKING_RATE} — 管线埋入墙内`,
    bracketCheck: "室外支架",
    ...
    startOver: "重新开始",
    note: `标准安装包含前${FREE_RUN_FEET}英尺铜管、保温层、电线和排水管。额外铜管按HP每英尺RM 17–27，电线每英尺RM 9，保温层每英尺RM 7，排水管每英尺RM ${DRAIN_PIPE_RATE}。小型PVC线槽（电线）每英尺RM 6；大型PVC线槽（铜管+电线+保温层）每英尺RM 12。打墙与隐藏式走管工程每英尺RM ${WALL_HACKING_RATE}。室外支架为付费附加项目（RM 45–70），不免费包含。多台批量折扣：5+台享5%折扣，10台以上享10%折扣。`,
...
export function InstallationCostCalculator({ lang = "en" }: { lang?: Lang }) {
  ...
  const waMsg = [
    ...
    wallHackingFeet > 0 ? `🧱 Wall hacking & concealment / Pecah dinding / 打墙隐藏走管: ${wallHackingFeet} ft` : "",
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <NumberField label={s.wallHacking} value={wallHackingFeet} onChange={(v) => { setWallHackingFeet(Math.max(0, Math.min(100, v))); setShowResult(false); }} min={0} max={100} suffix="ft" hint={s.wallHackingHint} />
