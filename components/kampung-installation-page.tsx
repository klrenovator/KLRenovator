...
export function KampungInstallationLandingPage({
  parentSlug,
  kampungSlug,
  locale,
}: {
  ...
  const c = getKampungInstallationContent(parentSlug, kampungSlug, locale);
  const basePath = locale === "en" ? "" : `/${locale}`;
  const pageUrl = `https://www.klrenovator.com${basePath}/areas/${parentSlug}/${kampungSlug}/installation`;
  const parentUrl = `https://www.klrenovator.com${basePath}/areas/${parentSlug}`;
  const areaName = c.h1.replace(/Aircond Installation |Pemasangan Aircond |冷气安装 /, "");
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: areaName, item: parentUrl },
      ...
      <div className="bg-slate-50 border-b border-slate-200">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-3">
          <nav className="flex items-center gap-1 text-xs text-slate-500" aria-label="Breadcrumb">
            ...
            <FiChevronRight className="h-3 w-3" />
            <Link href={`${basePath}/areas/${parentSlug}`} className="hover:text-sky-600 transition">{areaName}</Link>
            <FiChevronRight className="h-3 w-3" />
