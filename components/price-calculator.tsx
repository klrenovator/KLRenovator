...
export function PriceCalculator() {
  ...
  return <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
    ...
    <div className="p-6 space-y-4">
      <label className="block text-xs font-black uppercase tracking-widest text-slate-500">1. Service<Select value={service} onChange={(value) => { const nextService = value as ServiceType; setService(nextService); if (nextService === "chemical-overhaul" && unitType !== "wall") setUnitType("wall"); setShowResult(false); }}><option value="basic-servicing">Basic Servicing</option><option value="chemical-wash">Chemical Wash</option><option value="chemical-overhaul">Chemical Overhaul (Wall-Mounted only)</option><option value="gas-topup">Gas Top-Up</option><option value="installation">New Installation</option><option value="dismantle">Dismantle Only</option><option value="relocate-same">Relocate — Reinstall Same Place</option><option value="relocate-other">Relocate — Reinstall Other Place</option><option value="repair">Repair / Diagnostic</option></Select></label>
