...
const BASE_PRICES: Record<Exclude<ServiceType, "gas-topup" | "installation" | "relocate-same">, Record<string, number>> = {
  ...
  "chemical-overhaul": { "1.0-1.5": 420, "2.0-2.5": 490, "3.0-3.5": 560 },
...
function Select({ value, onChange, children }: { value: string; onChange: (value: string) => void; children: ReactNode }) {
  return <div className="relative"><select value={value} onChange={(e) => onChange(e.target.value)} className={selectCls}>{children}</select><FiChevronDown className="pointer-events-none absolute right-3.5 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-600" /></div>;
}
function FeetInput({ label, value, onChange, hint }: { label: string; value: number; onChange: (value: number) => void; hint: string }) {
  return <label className="block"><span className="block text-[11px] font-bold text-slate-700 mb-1">{label}</span><input type="number" min="0" max="100" value={value} onChange={(e) => onChange(Math.max(0, Math.min(100, Number(e.target.value) || 0)))} className={selectCls} /><span className="block text-[10px] text-slate-600 mt-1">{hint}</span></label>;
}
...
export function PriceCalculator() {
  ...
  return <div className="bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden">
    ...
    <div className="p-6 space-y-4">
      <label className="block text-xs font-black uppercase tracking-widest text-slate-600">1. Service<Select value={service} onChange={(value) => { const nextService = value as ServiceType; setService(nextService); if (nextService === "chemical-overhaul" && unitType !== "wall") setUnitType("wall"); setShowResult(false); }}><option value="basic-servicing">Basic Servicing</option><option value="chemical-wash">Chemical Wash</option><option value="chemical-overhaul">Chemical Overhaul (Wall-Mounted only)</option><option value="gas-topup">Gas Top-Up</option><option value="installation">New Installation</option><option value="dismantle">Dismantle Only</option><option value="relocate-same">Relocate — Reinstall Same Place</option><option value="relocate-other">Relocate — Reinstall Other Place</option><option value="repair">Repair / Diagnostic</option></Select></label>
      <div className="grid grid-cols-2 gap-3"><label className="block text-xs font-black uppercase tracking-widest text-slate-600">2. Unit Type<Select value={unitType} onChange={(value) => { setUnitType(value as UnitType); if (service === "chemical-overhaul" && value !== "wall") { setUnitType("wall"); return; } setShowResult(false); }}><option value="wall">Wall-Mounted</option>{service !== "chemical-overhaul" && <option value="cassette">Ceiling Cassette</option>}{service !== "chemical-overhaul" && <option value="window">Window Unit</option>}</Select></label><label className="block text-xs font-black uppercase tracking-widest text-slate-600">3. HP Size<Select value={hp} onChange={(value) => { setHp(value); setShowResult(false); }}><option value="1.0-1.5">1.0 – 1.5 HP</option><option value="2.0">2.0 HP</option><option value="2.5">2.5 HP</option><option value="3.0-3.5">3.0 – 3.5 HP</option></Select></label></div>
      {service === "gas-topup" && <label className="block text-xs font-black uppercase tracking-widest text-slate-600">Gas Type<Select value={gasType} onChange={(value) => { setGasType(value as GasType); setShowResult(false); }}><option value="r22">R22</option><option value="r410a">R410A</option><option value="r32">R32</option></Select></label>}
      <label className="block text-xs font-black uppercase tracking-widest text-slate-600">4. Number of Units<input type="number" min="1" max="20" value={units} onChange={(e) => { setUnits(Math.max(1, Math.min(20, Number(e.target.value) || 1))); setShowResult(false); }} className={selectCls} /></label>
      {includesInstallationMaterials && <div className="border-t border-slate-100 pt-4 space-y-4"><div><p className="text-xs font-black uppercase tracking-widest text-slate-600">5. Installation materials & add-ons</p><p className="text-[11px] text-emerald-700 mt-1 font-semibold">First 7 ft per unit is included free: copper pipe, insulation, electrical wire and drain pipe. Outdoor bracket is a paid optional item.</p></div><div className="grid grid-cols-2 gap-3"><FeetInput label="Copper Pipe Length" value={copperFeet} onChange={setCopperFeet} hint={`Extra: RM ${COPPER_RATE(hp)}/ft`} /><FeetInput label="Electrical Wire Length" value={wireFeet} onChange={setWireFeet} hint="Extra: RM 9/ft" /><FeetInput label="Insulation Length" value={insulationFeet} onChange={setInsulationFeet} hint="Extra: RM 7/ft" /><FeetInput label="Drain Pipe Length" value={drainFeet} onChange={setDrainFeet} hint="Extra: RM 5/ft" /><FeetInput label="Small PVC Casing (Electrical Wire)" value={smallPvcFeet} onChange={setSmallPvcFeet} hint="RM 6/ft" /><FeetInput label="Large PVC Casing (Copper + Wire + Insulation)" value={largePvcFeet} onChange={setLargePvcFeet} hint="RM 12/ft" /><FeetInput label="Wall Hacking & Concealment Work" value={wallHackingFeet} onChange={setWallHackingFeet} hint={`RM ${WALL_HACKING_RATE}/ft — concealed piping in wall`} /></div><label className="block text-[11px] font-bold text-slate-700">Outdoor Bracket (paid special charge only)<Select value={bracket} onChange={(value) => { setBracket(value as typeof bracket); setShowResult(false); }}><option value="none">No bracket needed</option><option value="standard">Standard Compressor / Outdoor Bracket — RM 45</option><option value="heavy">Heavy Duty Compressor / Outdoor Bracket — RM 70</option></Select></label></div>}
